...
func _input(event):
	if event.is_action_pressed("ui_up"):
		example += 1;
		if example > sample_gamestate.player_hand.size():
			example = 0
		print(sample_gamestate.player_hand[example].Rank, " ", sample_gamestate.player_hand[example].Suit)
	elif event.is_action_pressed("ui_down"):
		example -= 1;
		if example < 0:
			example = sample_gamestate.player_hand.size()
		print(sample_gamestate.player_hand[example].Rank, " ", sample_gamestate.player_hand[example].Suit)
	elif event.is_action_pressed("ui_right"):
		sample_gamestate = attack(sample_gamestate, sample_gamestate.player_hand[example])
		
	elif event.is_action_pressed("ui_left"):
		pass
	elif event.is_action_pressed("ui_accept"):
		sample_gamestate =  end_turn(sample_gamestate)
