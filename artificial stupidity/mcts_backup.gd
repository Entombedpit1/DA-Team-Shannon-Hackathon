...
class MctsNode:
	...
	func find_highest_avg_reward_action() -> Callable:
		...
		var highest_avg_reward:float = 1.0*highest_child.total_wins/max(highest_child.visit_count, 1)
		for child in self.children:
			var curr_avg:float = (1.0*child.total_wins/max(child.visit_count, 1))
...
func end_turn(game_state:GameState) -> GameState:
	...
	if (game_state.player_curr_health <= 0):
		...
		game_state.result_of_computer = -1;
		game_state.is_terminal = true;
	elif (game_state.draw_pile.is_empty() && (game_state.computer_hand.is_empty())):
		game_state.result_of_computer = 1;
		game_state.is_terminal = true;
	elif (game_state.draw_pile.is_empty() && (game_state.player_hand.is_empty())):
		game_state.result_of_computer = -1;
		game_state.is_terminal = true;
